...
export async function GET() {
  try {
    ...
    // Check if the business-permit-documents bucket exists
    ...
    const bucketExists = buckets?.some((bucket) => bucket.name === "business-permit-documents")

    if (!bucketExists) {
      // Create the bucket if it doesn't exist
      const { error } = await supabase.storage.createBucket("business-permit-documents", {
        public: false,
